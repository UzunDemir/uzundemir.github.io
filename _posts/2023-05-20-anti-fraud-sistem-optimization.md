...
#### Параметры моделей (Таблица 4):
| Модель             | Основные параметры                                   | Результат          |
|--------------------|------------------------------------------------------|--------------------|
| **Дерево решений** | Глубина: 15, min_samples_leaf: 50                   | F1-score: 0.72     |
| **Случайный лес**  | n_estimators: 100, max_depth: 20                    | F1-score: 0.78     |
| **Градиентный бустинг** | learning_rate: 0.1, n_estimators: 200       | **F1-score: 0.82** |

**Градиентный бустинг показал наилучшее качество**.

#### Статистика по 800 правилам (Таблица 5):
| Метрика               | Среднее значение | 95% доверительный интервал |
|------------------------|------------------|----------------------------|
| **Точность (precision)** | 48%             | [45%, 51%]                 |
| **Полнота (recall)**    | 0.7%            | [0.5%, 0.9%]               |
| **FPR**                | 12%             | [10%, 14%]                 |
...
Если нужно уточнить детали или продолжить перевод других разделов, дайте знать!
